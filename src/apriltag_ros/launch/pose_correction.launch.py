...
default_config_common = os.path.join(
    get_package_share_directory('husky_nav_bringup'),
    'config',
    'common_husky_nav_ekf.yaml'
)
...
config_path = get_package_share_directory('husky_nav_bringup')
...
def launch_setup(context, *args, **kwargs):
    ...
    wrapper_dir = get_package_share_directory('husky_nav_bringup')
    ...
    apriltag_correction_namespace = LaunchConfiguration('namespace')
    ...
    zed_wrapper_node = Node(
        ...
        # condition=IfCondition(PythonExpression(['"', LaunchConfiguration('run_mode'), '" == "zed_only" or "', '(', LaunchConfiguration('run_composed'), '" == "false" and', LaunchConfiguration('run_mode'), '" == "both" )"']))
    )

    # LINK - ZED Wrapper Component
    ...
    # LINK - Apriltag Components
    # Jackal image_rect topic =>  /j100_0921/sensors/camera_0/stereolabs_zed/left/image_rect_color
    # camera_info topic => /j100_0921/sensors/camera_0/stereolabs_zed/left/camera_info
    apriltag_correction_component = ComposableNode(
        package='apriltag_ros',
        namespace='/j100_0921',
        plugin='PoseCorrectionNode',
        name='pose_correction_node',
        remappings=[
            ('/tf', 'tf'),
            ('/tf_static', 'tf_static'),
            ('image_rect', 'sensors/camera_0/stereolabs_zed/left/image_rect_color'),
            ('camera_info', 'sensors/camera_0/stereolabs_zed/left/camera_info'),
            ('set_pose', 'unused_set_pose')
        ],
    ...
    # zed_apriltag_correction_container = ComposableNodeContainer(
    #     name='zed_apriltag_pose_correction',
    #     namespace=camera_name_val,
    #     package='rclcpp_components',
    #     executable='component_container',
    #     composable_node_descriptions=[
    #         zed_wrapper_component,
    #         apriltag_correction_component
    #     ],
    #     remappings=[
    #         ('/tf', 'tf'),
    #         ('/tf_static', 'tf_static'),
    #         ('image_rect', 'zed_node/left/image_rect_color'),
    #         ('camera_info', 'zed_node/left/camera_info')
    #     ],
    #     output='both',
    #     # condition=IfCondition(PythonExpression(['"', LaunchConfiguration('run_mode'), '" == "both" and "', LaunchConfiguration('run_composed'), '" == "true"']))
    # )

    # LINK - Component Container
    # Jackal image_rect topic =>  /j100_0921/sensors/camera_0/stereolabs_zed/left/image_rect_color
    # camera_info topic => /j100_0921/sensors/camera_0/stereolabs_zed/left/camera_info
    correction_only_container = ComposableNodeContainer(
        name='zed_apriltag_pose_correction',
        namespace="/j100_0921",
        package='rclcpp_components',
        executable='component_container',
        ...
        output='both',
        # condition=IfCondition(PythonExpression(['"', LaunchConfiguration('run_mode'), '" == "tag_only"']))
    )

    return [
        # rsp_node,
        # zed_apriltag_correction_container,
        correction_only_container,
        # zed_wrapper_node
    ]
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                'run_mode',
                default_value='both',
                description='Whether to run just the ZED wrapper, the pose correction node, or both. Valid options are: `zed_only`, `tag_only`, `both`'),
            DeclareLaunchArgument(
                'run_composed',
                default_value='true',
                description='If running both, whether to run them as separate processes or composed with memory sharing'
            ),
            DeclareLaunchArgument(
                'camera_name',
            ...
            DeclareLaunchArgument(
                ...
                default_value=os.path.join(
                    config_path, 'config', 'tags_36h11.yaml'),
